...
import {AsyncQueue, awaitWsReady, ionValueToObject, QueueClosedException} from "./minor/utils.js";
// @ts-ignore
//import {decode, encode} from "cbor-js/cbor.js";
import {generateKey, sign, readPrivateKey, createCleartextMessage, createMessage} from 'openpgp/lightweight';
import * as ion from "ion-js";
import PubSub from "pubsub-js";
import type {ProfileData} from "./profile.js";

export class ClientServerInstance {


}


export type ConnectionStatus = "disconnected" | "ws::connecting" | "connecting" | "connected" | "error";
...
export class V1ClientConnection {
    ...
    statusSnapshot: ConnectionStatus = "disconnected";
    readonly status: Writable<ConnectionStatus> = writable("disconnected");
    private readonly ready: Promise<void>
    private readonly statusSnapshotUpdaterUnsubscribe: () => void;

    constructor(readonly baseURL: string, readonly profile: ProfileData, readonly textMode: boolean = false) {
        let wsURLString = this.urlToWs(baseURL) + "/_zwie/01/socket" + (textMode ? "?text" : "");
        this.wsURL = new URL(wsURLString);

        this.status.set("ws::connecting")
        ...
        this.statusSnapshotUpdaterUnsubscribe = this.status.subscribe((status) => {
            console.debug("connection status update", status)
            this.statusSnapshot = status;
        })
    }

    private handleClose(event: CloseEvent) {
        console.debug("socket closed", event);//TODO: Handle this (reconnect?)
        this.status.set("disconnected")
        this.statusSnapshotUpdaterUnsubscribe();
    ...
    async runHandshake() {
        ...
        const handshakeTimeLabel = "handshake: " + this.baseURL
        console.time(handshakeTimeLabel);
        try {
            /*
            ...
            this.send("client/init/init1", {
                ...
                identity: this.profile.publicKey/*key.publicKey*/,
            })
            console.timeLog(handshakeTimeLabel, "wait for init2");
            ...
            //})
            let challengeMessage = await createMessage({
                format: "binary",
                binary: payload.challenge,
            })
            let singed;
            let key = await readPrivateKey({armoredKey: this.profile.privateKey /*key.privateKey */});
            if (this.textMode) {
                singed = await sign({
            ...
            console.timeEnd(handshakeTimeLabel);
        }
    ...
    async runLoop() {
        ...
        while (this.statusSnapshot === "connected") {
            try {
                let [type, payload] = await this.receive()
