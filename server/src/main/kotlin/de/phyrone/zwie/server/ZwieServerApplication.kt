...
class ZwieServerApplication {
    ...
    companion object Main {
        ...
        fun main(args: Array<String>) {
            ...
            if (System.getProperty("jansi.enable")?.toBooleanStrictOrNull() == true)
    ...
    fun jsonObjectMapper() = ObjectMapper().findAndRegisterModules().also { mapper ->
    ...
    fun cborObjectMapper() = CBORMapper().findAndRegisterModules().also { mapper ->
        mapper.registerSubtypes(ClassIndex.getSubclasses(JsonComponent::class.java).toSet())
        logger.atFine().log(
            "Registered %s json modules [%s]",
            lazyArg { mapper.registeredModuleIds.size },
